#!/usr/bin/env python3
"""Train a common 18D baseline for horizon=1 future-step prediction."""

from __future__ import annotations

import argparse
import json
import random
import sys
from pathlib import Path
from typing import Dict, List

import numpy as np
import torch
import torch.nn as nn
from torch.utils.data import DataLoader, TensorDataset, WeightedRandomSampler

PROJECT_ROOT = Path(__file__).resolve().parents[2]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from models_tcn import InceptionTimeClassifier  # noqa: E402
from training_utils import (  # noqa: E402
    compute_class_weights,
    evaluate,
    split_train_val_stratified,
    validate_forecast_dataset_contract,
)

SCRIPT_DIR = Path(__file__).resolve().parent
if str(SCRIPT_DIR) not in sys.path:
    sys.path.insert(0, str(SCRIPT_DIR))

from models_ablation import GRUClassifier, LSTMClassifier, TransformerEncoderClassifier  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a unified 18D baseline on the horizon=1 future-step dataset"
    )
    parser.add_argument(
        "--dataset-npz",
        type=Path,
        default=PROJECT_ROOT / "Data/processed_data/weld_seam_windows_ws5_tf75_pg0_h1.npz",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "ablation_experiments/h1/results/comparison_18d_baselines/lstm",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="lstm",
        choices=["lstm", "gru", "transformer", "inception"],
    )

    parser.add_argument("--epochs", type=int, default=80)
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--lr", type=float, default=2.5e-4)
    parser.add_argument("--weight-decay", type=float, default=2e-4)
    parser.add_argument("--label-smoothing", type=float, default=0.0)
    parser.add_argument("--seed", type=int, default=100)
    parser.add_argument("--num-workers", type=int, default=0)

    parser.add_argument("--class-weights", type=str, default="auto", help="auto | none | comma values")
    parser.add_argument("--weighted-sampler", dest="weighted_sampler", action="store_true")
    parser.add_argument("--no-weighted-sampler", dest="weighted_sampler", action="store_false")
    parser.add_argument("--val-ratio", type=float, default=0.15)
    parser.add_argument("--early-stop-patience", type=int, default=16)
    parser.add_argument("--min-epochs", type=int, default=16)
    parser.add_argument(
        "--checkpoint-metric",
        type=str,
        default="val_macro_f1",
        choices=["val_macro_f1", "test_acc"],
    )

    parser.add_argument("--rnn-hidden-size", type=int, default=64)
    parser.add_argument("--rnn-layers", type=int, default=2)
    parser.add_argument("--rnn-dropout", type=float, default=0.2)
    parser.add_argument("--rnn-bidirectional", dest="rnn_bidirectional", action="store_true")
    parser.add_argument("--no-rnn-bidirectional", dest="rnn_bidirectional", action="store_false")

    parser.add_argument("--transformer-d-model", type=int, default=96)
    parser.add_argument("--transformer-nhead", type=int, default=4)
    parser.add_argument("--transformer-layers", type=int, default=3)
    parser.add_argument("--transformer-ff-dim", type=int, default=256)
    parser.add_argument("--transformer-dropout", type=float, default=0.2)

    parser.add_argument("--inception-out-ch", type=int, default=16)
    parser.add_argument("--inception-blocks", type=int, default=6)
    parser.add_argument("--inception-bottleneck", type=int, default=16)
    parser.add_argument("--inception-dropout", type=float, default=0.2)

    parser.add_argument("--classifier-hidden", type=int, default=128)
    parser.add_argument("--classifier-dropout", type=float, default=0.35)
    parser.set_defaults(weighted_sampler=True, rnn_bidirectional=True)
    return parser.parse_args()


def load_npz_dataset(path: Path) -> Dict[str, np.ndarray]:
    with np.load(str(path), allow_pickle=False) as z:
        return {k: z[k] for k in z.files}


def set_seed(seed: int) -> None:
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False


def build_model(args: argparse.Namespace, input_dim: int, num_classes: int) -> nn.Module:
    if args.model == "lstm":
        return LSTMClassifier(
            input_dim=input_dim,
            num_classes=num_classes,
            hidden_size=args.rnn_hidden_size,
            num_layers=args.rnn_layers,
            dropout=args.rnn_dropout,
            bidirectional=args.rnn_bidirectional,
            classifier_hidden=args.classifier_hidden,
            classifier_dropout=args.classifier_dropout,
        )
    if args.model == "gru":
        return GRUClassifier(
            input_dim=input_dim,
            num_classes=num_classes,
            hidden_size=args.rnn_hidden_size,
            num_layers=args.rnn_layers,
            dropout=args.rnn_dropout,
            bidirectional=args.rnn_bidirectional,
            classifier_hidden=args.classifier_hidden,
            classifier_dropout=args.classifier_dropout,
        )
    if args.model == "transformer":
        return TransformerEncoderClassifier(
            input_dim=input_dim,
            num_classes=num_classes,
            d_model=args.transformer_d_model,
            nhead=args.transformer_nhead,
            num_layers=args.transformer_layers,
            ff_dim=args.transformer_ff_dim,
            dropout=args.transformer_dropout,
            classifier_hidden=args.classifier_hidden,
            classifier_dropout=args.classifier_dropout,
        )
    return InceptionTimeClassifier(
        input_dim=input_dim,
        num_classes=num_classes,
        out_ch=args.inception_out_ch,
        n_blocks=args.inception_blocks,
        bottleneck=args.inception_bottleneck,
        dropout=args.inception_dropout,
        classifier_hidden=args.classifier_hidden,
        classifier_dropout=args.classifier_dropout,
    )


def build_run_name(args: argparse.Namespace, dataset_tag: str) -> str:
    if args.model in {"lstm", "gru"}:
        direction = "bi" if args.rnn_bidirectional else "uni"
        backbone = f"h{args.rnn_hidden_size}_l{args.rnn_layers}_{direction}_do{args.rnn_dropout}"
    elif args.model == "transformer":
        backbone = (
            f"dm{args.transformer_d_model}_nh{args.transformer_nhead}_l{args.transformer_layers}_"
            f"ff{args.transformer_ff_dim}_do{args.transformer_dropout}"
        )
    else:
        backbone = (
            f"oc{args.inception_out_ch}_blk{args.inception_blocks}_bn{args.inception_bottleneck}_"
            f"do{args.inception_dropout}"
        )
    return (
        f"{args.model}_{dataset_tag}_{backbone}_ep{args.epochs}_lr{args.lr}_"
        f"wd{args.weight_decay}_bs{args.batch_size}_seed{args.seed}"
    )


def main() -> None:
    args = parse_args()
    set_seed(args.seed)

    default_lstm_output = PROJECT_ROOT / "ablation_experiments/h1/results/comparison_18d_baselines/lstm"
    if args.model != "lstm" and args.output_dir == default_lstm_output:
        args.output_dir = default_lstm_output.parent / args.model

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    validate_forecast_dataset_contract(args.dataset_npz)

    ds = load_npz_dataset(args.dataset_npz)
    x_train = ds["X_train_full"].astype(np.float32)
    y_train = ds["y_train"].astype(np.int64)
    x_test = ds["X_test_full"].astype(np.float32)
    y_test = ds["y_test"].astype(np.int64)

    num_classes = int(max(y_train.max(), y_test.max()) + 1)
    class_names = [f"Class {i}" for i in range(num_classes)]
    train_idx, val_idx = split_train_val_stratified(y_train, args.val_ratio, args.seed)
    x_tr = x_train[train_idx]
    y_tr = y_train[train_idx]
    x_val = x_train[val_idx]
    y_val = y_train[val_idx]

    model = build_model(args, input_dim=int(x_train.shape[2]), num_classes=num_classes).to(device)
    class_weights = compute_class_weights(y_tr, num_classes, args.class_weights)
    if class_weights is not None:
        class_weights = class_weights.to(device)

    loss_fn = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=float(args.label_smoothing))
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)

    train_dataset = TensorDataset(torch.tensor(x_tr), torch.tensor(y_tr, dtype=torch.long))
    val_dataset = TensorDataset(torch.tensor(x_val), torch.tensor(y_val, dtype=torch.long))
    test_dataset = TensorDataset(torch.tensor(x_test), torch.tensor(y_test, dtype=torch.long))

    if args.weighted_sampler:
        counts = np.bincount(y_tr, minlength=num_classes)
        sample_w = 1.0 / np.maximum(counts[y_tr], 1)
        sample_w = torch.tensor(sample_w, dtype=torch.double)
        sampler = WeightedRandomSampler(weights=sample_w, num_samples=len(sample_w), replacement=True)
        train_loader = DataLoader(
            train_dataset,
            batch_size=args.batch_size,
            sampler=sampler,
            num_workers=args.num_workers,
        )
    else:
        train_loader = DataLoader(
            train_dataset,
            batch_size=args.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
        )

    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)

    args.output_dir.mkdir(parents=True, exist_ok=True)
    dataset_tag = args.dataset_npz.stem
    run_name = build_run_name(args, dataset_tag)
    run_dir = args.output_dir / run_name
    run_dir.mkdir(parents=True, exist_ok=True)

    run_payload = {k: (str(v) if isinstance(v, Path) else v) for k, v in vars(args).items()}
    run_payload["resolved_output_dir"] = str(run_dir)
    with (run_dir / "run_args.json").open("w", encoding="utf-8") as f:
        json.dump(run_payload, f, indent=2, ensure_ascii=True)

    print("=" * 80)
    print(f"Comparison baseline | model={args.model}")
    print("=" * 80)
    print(f"Dataset: {args.dataset_npz}")
    print(f"Device: {device}")
    print(f"Train/Val/Test: {len(x_tr)} / {len(x_val)} / {len(x_test)}")

    best_score = -1.0
    best_epoch = -1
    best_state: Dict[str, torch.Tensor] | None = None
    wait = 0
    history: List[Dict[str, float]] = []

    for epoch in range(1, args.epochs + 1):
        model.train()
        total_loss = 0.0
        n_batches = 0

        for xb, yb in train_loader:
            xb = xb.to(device)
            yb = yb.to(device)

            optimizer.zero_grad()
            logits, _ = model(xb)
            loss = loss_fn(logits, yb)
            loss.backward()
            optimizer.step()

            total_loss += float(loss.item())
            n_batches += 1

        scheduler.step()
        train_loss = total_loss / n_batches if n_batches else 0.0
        val_eval = evaluate(model, val_loader, device, loss_fn, class_names)
        test_eval = evaluate(model, test_loader, device, loss_fn, class_names)
        history.append(
            {
                "epoch": epoch,
                "train_loss": train_loss,
                "val_loss": val_eval.loss,
                "val_acc": val_eval.accuracy,
                "val_macro_f1": val_eval.macro_f1,
                "test_acc": test_eval.accuracy,
                "test_macro_f1": test_eval.macro_f1,
                "lr": float(optimizer.param_groups[0]["lr"]),
            }
        )

        print(
            f"Epoch {epoch:03d} | train_loss={train_loss:.4f} | "
            f"val_acc={val_eval.accuracy*100:.2f}% val_f1={val_eval.macro_f1:.4f} | "
            f"test_acc={test_eval.accuracy*100:.2f}% test_f1={test_eval.macro_f1:.4f}"
        )

        current_score = val_eval.macro_f1 if args.checkpoint_metric == "val_macro_f1" else test_eval.accuracy
        if current_score > best_score:
            best_score = current_score
            best_epoch = epoch
            best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
            wait = 0
        else:
            wait += 1

        if epoch >= args.min_epochs and wait >= args.early_stop_patience:
            print(f"Early stopping at epoch {epoch} (best={best_epoch}, score={best_score:.6f})")
            break

    if best_state is None:
        raise RuntimeError("Training did not produce a valid checkpoint")

    model.load_state_dict(best_state)
    model.to(device)

    train_eval = evaluate(
        model,
        DataLoader(
            TensorDataset(torch.tensor(x_train), torch.tensor(y_train, dtype=torch.long)),
            batch_size=args.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
        ),
        device,
        loss_fn,
        class_names,
    )
    val_eval = evaluate(model, val_loader, device, loss_fn, class_names)
    test_eval = evaluate(model, test_loader, device, loss_fn, class_names)

    torch.save(model.state_dict(), run_dir / "best_model.pth")
    with (run_dir / "history.json").open("w", encoding="utf-8") as f:
        json.dump(history, f, indent=2, ensure_ascii=True)

    with (run_dir / "evaluation_metrics.txt").open("w", encoding="utf-8") as f:
        f.write(f"Best epoch: {best_epoch}\n")
        f.write(f"Checkpoint metric: {args.checkpoint_metric}\n")
        f.write(f"Best score: {best_score:.6f}\n\n")

        f.write(f"--- Train Metrics ({args.model}) ---\n")
        f.write(f"Loss: {train_eval.loss:.6f}\n")
        f.write(f"Accuracy: {train_eval.accuracy*100:.2f}%\n")
        f.write(f"Macro-F1: {train_eval.macro_f1:.6f}\n")
        f.write("\n=== Classification Report (Train) ===\n")
        f.write(train_eval.report + "\n\n")

        f.write(f"--- Val Metrics ({args.model}) ---\n")
        f.write(f"Loss: {val_eval.loss:.6f}\n")
        f.write(f"Accuracy: {val_eval.accuracy*100:.2f}%\n")
        f.write(f"Macro-F1: {val_eval.macro_f1:.6f}\n")
        f.write("\n=== Classification Report (Val) ===\n")
        f.write(val_eval.report + "\n\n")

        f.write(f"--- Test Metrics ({args.model}) ---\n")
        f.write(f"Loss: {test_eval.loss:.6f}\n")
        f.write(f"Accuracy: {test_eval.accuracy*100:.2f}%\n")
        f.write(f"Macro-F1: {test_eval.macro_f1:.6f}\n")
        f.write("\n=== Classification Report (Test) ===\n")
        f.write(test_eval.report + "\n")

    print("=" * 80)
    print(f"Best epoch: {best_epoch}")
    print(f"Best score ({args.checkpoint_metric}): {best_score:.6f}")
    print(f"Final test accuracy: {test_eval.accuracy*100:.2f}%")
    print(f"Saved to: {run_dir}")


if __name__ == "__main__":
    main()
