#!/usr/bin/env python3
"""Ablation-2 stage1: train 18D LSTM teacher for future-step prediction."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ablation-2 stage1: train LSTM teacher for future-step prediction"
    )
    parser.add_argument(
        "--dataset-npz",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "ablation_experiments/h1/results/ablation2_teacher_student_lstm/teachers",
    )
    ...
    parser.add_argument("--lstm-bidirectional", dest="lstm_bidirectional", action="store_true")
    parser.add_argument("--no-lstm-bidirectional", dest="lstm_bidirectional", action="store_false")
...
def main() -> None:
    ...
    print("=" * 80)
    print("Ablation-2 | Teacher (18D) future-step prediction with LSTM")
    print("=" * 80)
    ...
    torch.save(model.state_dict(), run_dir / "best_teacher_lstm.pth")
    ...
    with (run_dir / "evaluation_metrics.txt").open("w", encoding="utf-8") as f:
        ...
        f.write("--- Train Metrics (Teacher LSTM, future-step prediction) ---\n")
        ...
        f.write("--- Val Metrics (Teacher LSTM, future-step prediction) ---\n")
        ...
        f.write("--- Test Metrics (Teacher LSTM, future-step prediction) ---\n")
